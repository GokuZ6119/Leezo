...
module.exports.run = async(client, message, args) => {
  ...
  if(message.content.split(" ")[0] == CustomPrefix + 'suck'){ 
     message.delete()
    if (!message.channel.nsfw) {message.channel.send("🔞 Veuillez aller dans un salon **NSFW** 🔞"); return;}
    const embed = new Discord.MessageEmbed();
    const { body } = await superagent.get("https://waifu.pics/api/nsfw/blowjob")
    if(message.mentions.users.first() && message.mentions.users.first().id != message.author.id) {
    embed.setColor("#FFFF00")
    embed.setTitle(`${message.author.username} suce ${message.mentions.users.first().username} 🔞`)
    embed.setImage(body.url); } else {
    embed.setColor("#FFFF00")
    embed.setTitle(`$${message.author.username} | ***Leezo***  te suce 🔞`)
